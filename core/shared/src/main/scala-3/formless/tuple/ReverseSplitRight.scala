...
object ReverseSplitRight {
  ...
  given tupleReverseSplitRight[L <: Tuple, U](
    using f: FindField[ReverseT[L], U],
  ): ReverseSplitRight.Aux[L, U, U *: f.Tail, ReverseT[f.Head]] =
    new ReverseSplitRight[L, U] {
      ...
      def apply(l: L): Out = {
        ...
        (
          Tuple.fromArray(a(f.index) +: a.drop(f.index + 1)).asInstanceOf[Prefix],
