...
sealed trait RemoveLP {
  ...
  final given removeHCons[H, T <: HList, E, OutT <: HList](
...
object Remove extends RemoveLP {
  ...
  given removeHead[K, V, T <: HList]: Remove.Aux[(K ->> V) :: T, (K ->> V), ((K ->> V), T)] =
  ...
  given removeUnlabelledHead[K, V, T <: HList]: Remove.Aux[(K ->> V) :: T, V, (V, T)] =
    new Remove[(K ->> V) :: T, V] {
      type Out = (V, T)
      def apply(l: (K ->> V) :: T): Out = (l.head, l.tail)
